...
#include "authd_client_internal.h"
...
static pid_t authd_owner_pid;
static uid_t authd_owner_uid;
...
{
    return leonos_authd_client_wait_fd(fd, expected, payload, capacity, length, NULL);
}
                              ...
                              uint32_t capacity, uint32_t *length, int *received_fd)
{
    ...
    for (;;) {
        ...
        uint32_t got = 0;
        int ancillary = -1;
        if (leonos_ipc_recv_fd(fd, &type, buffer, sizeof(buffer), &got, &ancillary) == 0) {
            if (ancillary >= 0 && (!received_fd || type != expected ||
                got > capacity || (!length && got != capacity) || type == LEONOS_AUTHD_MSG_ACK)) {
                close(ancillary);
                ancillary = -1;
            }
            ...
            if (type == expected) {
                ...
                if (received_fd) *received_fd = ancillary;
                return 0;
            }
            errno = EPROTO;
            return -1;
...
{
    struct leonos_authd_hello hello = {.pid = (uint32_t)getpid()};
    struct leonos_authd_ack ack;
    struct ucred peer;
    int fd = leonos_ipc_connect(LEONOS_IPC_SOCK_AUTH);
    if (fd < 0) return -1;
    if (leonos_ipc_peer_credentials(fd, &peer) < 0 || peer.uid != 0) {
        leonos_ipc_close(fd);
        errno = EACCES;
        return -1;
    }
    if (leonos_ipc_set_nonblock(fd, 1) < 0 ||
        leonos_ipc_send(fd, LEONOS_AUTHD_MSG_HELLO, &hello, sizeof(hello)) < 0 ||
        leonos_authd_client_wait(fd, LEONOS_AUTHD_MSG_ACK, &ack, sizeof(ack), NULL) < 0) {
        int saved = errno;
        leonos_ipc_close(fd);
        errno = saved;
        return -1;
...
int leonos_authd_client_open(void)
{
    ...
    struct leonos_authd_ack ack;
    uint32_t deadline;
    if (authd_fd >= 0 && (authd_owner_pid != getpid() || authd_owner_uid != geteuid()))
    ...
    struct ucred peer;
    if (leonos_ipc_peer_credentials(authd_fd, &peer) < 0 || peer.uid != 0) {
        leonos_authd_client_close();
        errno = EACCES;
        return -1;
    }
    ...
    if (leonos_ipc_send(authd_fd, LEONOS_AUTHD_MSG_HELLO, &hello,
                        sizeof(hello)) < 0 ||
        leonos_authd_client_wait(authd_fd, LEONOS_AUTHD_MSG_ACK, &ack, sizeof(ack), 0) < 0) {
        leonos_ipc_close(authd_fd);
        authd_fd = -1;
        authd_retry_after_ms = authd_now_ms() + AUTHD_CONNECT_BACKOFF_MS;
        return -1;
    }
    authd_owner_pid = getpid();
    authd_owner_uid = geteuid();
    return authd_fd;
}
...
int leonos_auth_status(struct leonos_auth_status *status)
{
    ...
    if (leonos_authd_client_open() < 0) return -1;
    if (leonos_ipc_send(authd_fd, LEONOS_AUTHD_MSG_STATUS, 0, 0) < 0) return -1;
    return leonos_authd_client_wait(authd_fd, LEONOS_AUTHD_MSG_STATUS, status, sizeof(*status), 0);
}
...
int leonos_auth_current(struct leonos_user_info *user)
{
    ...
    return leonos_authd_client_wait(authd_fd, LEONOS_AUTHD_MSG_CURRENT, user, sizeof(*user), 0);
}
                           ...
                           uint32_t include_disabled, uint32_t *out_count)
{
    ...
    if (leonos_authd_client_wait(authd_fd, LEONOS_AUTHD_MSG_LIST, buffer, sizeof(buffer), &length) < 0) return -1;
                      ...
                      struct leonos_user_info *user)
{
    ...
    if (leonos_authd_client_wait(authd_fd, LEONOS_AUTHD_MSG_LOGIN, user, sizeof(*user), 0) < 0) return -1;
    /* Account lookup and cwd belong to login, not the kernel setuid syscall. */
    errno = 0;
    struct passwd *account = getpwnam(user->username);
    if (!account || account->pw_uid != user->uid) {
        if (!errno) errno = EACCES;
    }
    if (getuid() != 0) errno = EPERM;
    if (!account || account->pw_uid != user->uid || getuid() != 0 ||
        initgroups(account->pw_name, account->pw_gid) < 0 ||
        setgid(account->pw_gid) < 0 || setuid(account->pw_uid) < 0 || chdir(user->home) < 0) {
        int error = errno;
                               ...
                               struct leonos_user_info *user)
{
    ...
    if (leonos_ipc_send(authd_fd, LEONOS_AUTHD_MSG_ELEVATE, &login,
                        sizeof(login)) < 0) return -1;
    memset(login.password, 0, sizeof(login.password));
    return leonos_authd_client_wait(authd_fd, LEONOS_AUTHD_MSG_ELEVATE, user, sizeof(*user), 0);
}
...
int leonos_auth_logout(void)
{
    ...
    struct leonos_authd_ack ack;
    return leonos_authd_client_wait(authd_fd, LEONOS_AUTHD_MSG_ACK, &ack, sizeof(ack), 0);
}
                            ...
                            uint32_t role, struct leonos_user_info *user)
{
    ...
    return leonos_authd_client_wait(authd_fd, LEONOS_AUTHD_MSG_CREATE, user, sizeof(*user), 0);
}
