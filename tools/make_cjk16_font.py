...
def render_char(ch: str, font: str, point_size: int) -> bytes:
    cmd = [
        "convert",
        "-background",
        "black",
...
def main() -> int:
    ...
    cps = sorted({ord(ch) for ch in args.chars if ord(ch) >= 0x80})
    header_size = 24
    ...
    for cp in cps:
        ...
        bitmaps += render_char(chr(cp), args.font, args.point_size)
