...
#define LEONMMCOSSET_IMAGE_PATH "leonmmcoset.png"
#define LEONMMCOSSET_IMAGE_MARGIN 8U

static uint32_t pixels[LEONMMCOSSET_W * LEONMMCOSSET_H];
...
static int change_to_executable_directory(const char *path)
{
    ...
    if (!path || !path[0]) {
        return chdir("/programs/leonmmcoset");
    }
    while (path[length]) {
        if (path[length] == '/') {
...
static void draw_image(struct leonos_ui_surface *ui)
{
    ...
    draw_width = LEONMMCOSSET_W - LEONMMCOSSET_IMAGE_MARGIN * 2U;
    draw_height = (uint32_t)(((uint64_t)draw_width * image_height) / image_width);
    if (draw_height > LEONMMCOSSET_H - LEONMMCOSSET_IMAGE_MARGIN * 2U) {
        draw_height = LEONMMCOSSET_H - LEONMMCOSSET_IMAGE_MARGIN * 2U;
        draw_width = (uint32_t)(((uint64_t)draw_height * image_width) / image_height);
    }
    if (!draw_width || !draw_height) {
        return;
    }
    draw_x = (LEONMMCOSSET_W - draw_width) / 2U;
    draw_y = (LEONMMCOSSET_H - draw_height) / 2U;
...
int main(int argc, char **argv)
{
    ...
    if (leonos_png_decode_file(LEONMMCOSSET_IMAGE_PATH, &image_pixels,
        ...
        printf("[leonmmcoset.elf] relative PNG open path=%s size=%dx%d\n",
