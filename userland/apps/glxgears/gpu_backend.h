...
{
    gpu_sdk_context_t context = {0};
    int result;

    gears_gpu_release_target(gpu);
    if (width <= 0 || width > (int)GPU_SDK_MAX_WIDTH ||
        height <= 0 || height > (int)GPU_SDK_MAX_HEIGHT)
        return -22;
    gpu->pixels = calloc((size_t)width * (size_t)height, sizeof(*gpu->pixels));
    if (!gpu->pixels)
        return -12;
    context.size = sizeof(context);
    context.version = GPU_SDK_ABI_VERSION;
    context.width = (uint32_t)width;
    context.height = (uint32_t)height;
    context.vertex_capacity = gpu->vertex_count;
    result = gpu_sdk_create(&context);
...
{
    gpu_sdk_info_t info = {0};
    int result;

    info.size = sizeof(info);
    info.version = GPU_SDK_ABI_VERSION;
    result = gpu_sdk_info(&info);
    if (result < 0) {
    ...
    if (!(info.flags & GPU_SDK_AVAILABLE)) {
...
{
    gpu_sdk_draw_t *draw = &gpu->draws[index];
...
static int gears_gpu_draw(struct gears_gpu_backend *gpu)
{
    ...
    frame.version = GPU_SDK_ABI_VERSION;
    ...
    frame.fill_mode = (uint32_t)polygon_mode + GPU_SDK_FILL_POINT;
    return gpu_sdk_render(&frame);
}
