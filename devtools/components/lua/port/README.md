...
`/opt/lua/lua.elf`, backed by ABI-v1 `liblua.so.5` at
`/usr/lib/liblua.so.5`. The SDK also provides `liblua.a` for `STATIC=1`
applications.
...
`/opt/lua/lua/`. Dynamic C modules, `package.loadlib`, POSIX-only
